...
template<int dims> 
__global__ void _Euler_add_surface_term_kernel(field_type *rho, float *rho_der, float dx) {
...
void EulerCUDA<dims>::evolve() {
    ...
    _Euler_add_surface_term_kernel<dims><<<blocks, BLOCK_SIZE>>>(this->_d_rho, this->_d_rho_der, this->_dx);
