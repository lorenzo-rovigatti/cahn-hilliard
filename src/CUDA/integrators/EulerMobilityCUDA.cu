...
template<int dims> 
__global__ void _EulerMobility_add_surface_term_kernel(field_type *rho, float *rho_der, float dx) {
    if(IND >= c_grid_size[0]) return;
    
    int species = IND / c_grid_size[0];
    rho_der[IND] -= 2.f * c_k_laplacian[species] * _cell_laplacian<dims>(c_size[0], c_N[0], c_bits[0], rho, IND, dx);
}
...
void EulerMobilityCUDA<dims>::evolve() {
    ...
    _EulerMobility_add_surface_term_kernel<dims><<<blocks, BLOCK_SIZE>>>(this->_d_rho, this->_d_rho_der, this->_dx);
