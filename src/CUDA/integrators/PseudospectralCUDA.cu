...
PseudospectralCUDA<dims>::PseudospectralCUDA(SimulationState<dims> &sim_state, FreeEnergyModel *model, toml::table &config) : 
    ...
    if(dims == 1) {
        ...
        for(int species = 0; species < model->N_species(); species++) {
            for(int kx_idx = 0; kx_idx < reciprocal_n[0]; kx_idx++) {
                int kx_i = (kx_idx < reciprocal_n[0] / 2) ? kx_idx : (kx_idx - reciprocal_n[0]);
                double kx = 2.0 * M_PI * kx_i / (this->_N_per_dim * this->_dx);
                
                for(int ky_idx = 0; ky_idx < reciprocal_n[1]; ky_idx++) {
                    int ky_i = (ky_idx < reciprocal_n[1] / 2) ? ky_idx : (ky_idx - reciprocal_n[1]);
                    double ky = 2.0 * M_PI * ky_i / (this->_N_per_dim * this->_dx);
                    
                    for(int kz_idx = 0; kz_idx < (reciprocal_n[2] / 2 + 1); kz_idx++) {
