...
IMobility<dims> *build_mobility(toml::table &config, SimulationState<dims> &state) {
    ...
    if(mobility_type == "constant") {
        return new ConstantMobility<dims>(state, M);
    }
    else if(mobility_type == "free_energy") {
        return new FreeEnergyMobility<dims>(state, M);
    }
        ...
        return new GelMobility<dims>(state, dt, phi_critical, c_0, M_c, beta_delta_F);
    }
        ...
        return new RegularisedMobility<dims>(state, M, rho_min);
    }

    throw std::runtime_error("Unknown mobility type: " + mobility_type);
}

template IMobility<1> *build_mobility(toml::table &, SimulationState<1> &);
template IMobility<2> *build_mobility(toml::table &, SimulationState<2> &);
template IMobility<3> *build_mobility(toml::table &, SimulationState<3> &);
